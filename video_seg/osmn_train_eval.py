...
def read_annotation_list(text_file):
    ...
    ant_idx.append(int(read_list[1]))
    ant_file.append(read_list[2])
...
for name in train_seq_names:
    ...
    for label_id in label_fds:
        ...
        if randomize_guide:
            ...
            #ant_idx, ant_file = read_annotation_list(os.path.join('/home/jiangsihao/rawData',name,'frame_selection','BN0.txt')) 
            #print(name)
            #print(ant_file[0])           
            #train_imgs_with_guide += [(os.path.join(baseDirImg, name, ant_file[0]),
            #    os.path.join(baseDirLabel, name, label_id, ant_file[0][0:5] + '.png'),
            #    os.path.join(baseDirLabel, name, label_id, prev_frame[:-4] + '.png'),
            #    os.path.join(baseDirImg, name, frame),
            #    os.path.join(baseDirLabel, name, label_id, frame[:-4] + '.png'))
            #    for prev_frame, frame in zip(train_frames[:-1], train_frames[1:])]
