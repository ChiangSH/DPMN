...
from model_func import load_model, interp_surgery, osmn, osmn_deeplab, visual_modulator
...
def lovasz_hinge(logits, labels, per_image=True, ignore=None):
    ...
    if per_image:
        def treat_image(log_lab):
            ...
            #log = tf.transpose(log, (3, 0, 1, 2))
            #lab = tf.transpose(lab, (3, 0, 1, 2))
...
def class_balanced_cross_entropy_loss(output, label):
    ...
    per = tf.cast(tf.greater(output, 0.5), tf.float32)
    num_per = tf.reduce_sum(tf.multiply(labels,per))
    num_cha = tf.reduce_sum(labels)+tf.reduce_sum(per)-num_per
    bizhi = num_cha//num_per
    ...
    loss_per = lovasz_hinge(output, labels)
