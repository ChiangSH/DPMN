...
class Dataset:
    def __init__(self, train_anno_file, test_anno_file, train_image_path, test_image_path, args, data_aug=False):
        ...
        if os.path.exists('cache/train_annos.pkl'):
            self.train_annos = pk.load(open('cache/train_annos.pkl', 'rb'))
            self.test_annos = pk.load(open('cache/val_annos.pkl', 'rb'))
        else:
            ...
            if not os.path.exists('cache'):
                os.makedirs('cache')
            pk.dump(self.train_annos, open('cache/train_annos.pkl', 'wb'), pk.HIGHEST_PROTOCOL)
            pk.dump(self.test_annos, open('cache/val_annos.pkl', 'wb'), pk.HIGHEST_PROTOCOL)
        # Init parameters
